...
struct llama_quantization_data {
    std::vector<no_init<float>> f32_conv_buf;
    std::vector<std::thread> workers;
    llama_imatrix_data * imatrix_data = nullptr;
    size_t nthread;
    struct LLM_TN tn;

    llama_quantization_data(size_t nthread, llm_arch arch) : nthread(nthread), tn(arch) {
        f32_conv_buf.reserve(1024*1024);
        workers.reserve(nthread);
    }
};

struct llama_quantization_data * llama_model_quantization_create(size_t nthread, const char * arch) {
    return new llama_quantization_data(nthread, llm_arch_from_string(arch));
}
...
size_t llama_model_quantize_tensor(
    ...
    // size_t output_size)
{
    GGML_ASSERT(qd->imatrix_data == nullptr);

    // struct ggml_tensor tensor;
    // strcpy(tensor.name, name);
    // tensor.type = input_type;
    // memcpy(tensor.ne, ne, sizeof(tensor.ne));
    // memcpy(tensor.nb, nb, sizeof(tensor.nb));
    // tensor.data = (void *)input_data;


    const float * imatrix = nullptr;
    if (qd->imatrix_data) {
        auto it = qd->imatrix_data->find(tensor->name);
        if (it == qd->imatrix_data->end()) {
            ...
            if (it->second.size() == (size_t)tensor->ne[0]*tensor->ne[2]) {
                ...
                if (tensor->name != qd->tn(LLM_TENSOR_TOKEN_EMBD, "weight")) {
    ...
    if ((new_type == GGML_TYPE_IQ2_XXS ||
        ...
        (new_type == GGML_TYPE_Q2_K && ftype == LLAMA_FTYPE_MOSTLY_Q2_K_S && strcmp(tensor->name, "token_embd.weight") != 0)) && !imatrix) {
        LLAMA_LOG_ERROR("\n\n============================================================\n");
    ...
    //     throw std::runtime_error(format("requantizing from type %s is disabled", ggml_type_name(tensor->type)));
        ...
        llama_tensor_dequantize_internal(tensor, qd->f32_conv_buf, qd->workers, nelements, qd->nthread);
        f32_data = (float *) qd->f32_conv_buf.data();
    }
    ...
    const int64_t nthread_use = qd->nthread > 1 ? std::max((int64_t)1, std::min((int64_t)qd->nthread, nchunk)) : 1;
    ...
    for (int64_t i03 = 0; i03 < tensor->ne[2]; ++i03) {
        ...
        new_size += llama_tensor_quantize_internal(new_type, f32_data_03, new_data_03, chunk_size, nrows, n_per_row, imatrix_03, qd->workers, nthread_use);
    }
...
static void llama_model_quantize_internal(const std::string & fname_inp, const std::string & fname_out, const llama_model_quantize_params * params) {
    ...
    auto qd = new llama_quantization_data(nthread, model.arch);
    ...
    for (int i = 0; i < ml.n_tensors; ++i) {
        ...
        if (!quantize) {
            ...
            new_size = llama_model_quantize_tensor(qd, model.ftype, tensor, new_type, new_data);

...
server_ws_context::server_ws_context(const common_params & params)
...
bool server_ws_context::Impl::validate_api_key(const std::string & auth_header) const {
    ...
    // Extract the API key from the Authorization header (remove "Bearer " prefix if present)
    ...
    return std::find(api_keys.begin(), api_keys.end(), req_api_key) != api_keys.end();
}

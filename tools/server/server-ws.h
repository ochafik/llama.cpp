...
struct server_ws_connection {
    ...
    // User data - caller can attach anything (e.g., subprocess for MCP stdio)
...
struct server_ws_context {
    ...
    // Constructor takes params for configuration
    explicit server_ws_context(const common_params & params);
    ~server_ws_context();

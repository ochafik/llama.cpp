...
llama_grammar_stack llama_grammar_parser::c_rules() const {
    ...
    ret.stack.reserve(rules.size());
    for (const auto & rule : rules) {
        ret.stack.push_back(rule.data());
    }
    return ret;
}
...
static bool llama_grammar_advance_stack(
                              const llama_grammar_rules  & rules,
                                    llama_grammar_stack  & stack,
        const std::function<bool(llama_grammar_stack &)> & callback) {
    if (stack.stack.empty()) {
        return callback(stack);
    }

    const llama_grammar_element * pos = stack.stack.back();
    ...
    switch (pos->type) {
        case LLAMA_GRETYPE_RULE_REF: {
            ...
            do {
                ...
                auto should_continue = llama_grammar_advance_stack(rules, stack, callback);
...
static bool llama_grammar_accept_candidate_for_stack(
    ...
    std::function<bool(llama_grammar_stack &, const llama_grammar_candidate &)> accept_stack = [&](llama_grammar_stack & stack, const llama_grammar_candidate & tok) {
        ...
        llama_grammar_advance_stack(rules, stack, [&](llama_grammar_stack & stack) {
            ...
            if (accept_stack(stack, next_candidate)) {
                found = true;
                return false;
            }
            return true;
...
static llama_grammar_candidates llama_grammar_reject_candidates(
    ...
    // TODO:
    // - Ensure stacks are ordered by least pending tokens first (active stacks first)
...
void llama_grammar_accept(struct llama_grammar * grammar, uint32_t chr) {
    ...
    for (auto & stack : grammar->stacks) {
        ...
        if (lazy_stacks && !stacks_new.empty()) {
            ...
            if (match.first) {
                if (stack.pending_chars.empty() && (match.second->type == LLAMA_GRETYPE_CHAR || match.second->type == LLAMA_GRETYPE_CHAR_ANY || match.second->type == LLAMA_GRETYPE_CHAR_NOT)) {
                    // Trivial to advance, let's not add pending chars
                    fprintf(stderr, "T");
                    // fflush(stdout);
    ...
    // Sort by least pending chars first
    std::sort(grammar->stacks.begin(), grammar->stacks.end(), [](const auto & a, const auto & b) {
        return a.pending_chars.size() < b.pending_chars.size();
    });
...
llama_grammar_candidates llama_grammar_reject_candidates_for_stack(
    ...
    const llama_grammar_element * stack_pos = stack.stack.back();
    ...
    // update top of stack to next element, if any
    llama_grammar_stack stack_after;
    stack_after.stack = {stack.stack.begin(), stack.stack.end() - 1};
    if (!llama_grammar_is_end_of_sequence(stack_pos_after)) {
        stack_after.stack.push_back(stack_pos_after);
    }
...
struct llama_grammar * llama_grammar_init_impl(
    ...
    do {
        ...
        llama_grammar_advance_stack(vec_rules, stack, advance_callback);
...
struct llama_grammar * llama_grammar_init_impl(
    ...
    std::vector<const llama_grammar_element *> grammar_rules(parser.c_rules().stack);
...
struct llama_grammar * llama_grammar_clone_impl(const struct llama_grammar & grammar) {
    ...
    for (size_t is = 0; is < result->stacks.size(); is++) {
        for (size_t ie = 0; ie < result->stacks[is].stack.size(); ie++) {
            for (size_t ir0 = 0; ir0 < grammar.rules.size(); ir0++) {
                for (size_t ir1 = 0; ir1 < grammar.rules[ir0].size(); ir1++) {
                    if (grammar.stacks[is].stack[ie] == &grammar.rules[ir0][ir1]) {
                        result->stacks[is].stack[ie] =  &result->rules[ir0][ir1];
                    }
                }
            }
        }

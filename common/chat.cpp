...
const bool constrain_python_tool_code = !getenv("CONSTRAIN_PYTHON_TOOL_CODE") || std::string(getenv("CONSTRAIN_PYTHON_TOOL_CODE")) != "0";
...
static std::string add_json_tool_args_rule(const std::string & name, const json & parameters, const common_grammar_builder & builder) {
    if (constrain_python_tool_code && name == "python" && parameters.contains("properties") && parameters.at("properties").contains("code") && parameters.at("properties").size() == 1) {
    ...
    data.grammar = build_grammar([&](const common_grammar_builder & builder) {
        ...
        auto handle_builtin_tool = [&](const std::string & name, const json & parameters) {
            ...
            for (const auto & [key, value] : parameters.at("properties").items()) {
                if (constrain_python_tool_code && name == "python" && key == "code") {
        ...
        foreach_function(inputs.tools, [&](const json & tool) {
            ...
            tool_rules.push_back(
                builder.add_rule(
                    ...
                    "  \"\\\"parameters\\\"\" space \":\" space " + add_json_tool_args_rule(name, parameters, builder) + " "
                    "\"}\" space"));
            data.grammar_triggers.push_back({

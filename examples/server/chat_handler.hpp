...
class ChatHandler {
    ...
    std::string name_;
    json model_context;
    std::unique_ptr<jinja::TemplateNode> chat_prologue_;
    std::unique_ptr<jinja::TemplateNode> chat_template_;
    std::unique_ptr<jinja::TemplateNode> chat_epilogue_;

    std::unique_ptr<jinja::TemplateNode> tool_call_system_template_;
    std::unique_ptr<jinja::TemplateNode> tool_call_grammar_template_;
...
public:
    ...
    ChatHandler(const std::string & name, const json & model_context, const json & handler) : name_(name), model_context(model_context)
    {
        ...
        if (!chat_prologue.empty()) chat_prologue_ = jinja::Parser::parse(chat_prologue);
        if (!chat_template.empty()) chat_template_ = jinja::Parser::parse(chat_template);
        if (!chat_epilogue.empty()) chat_epilogue_ = jinja::Parser::parse(chat_epilogue);

        // if (!tool_call_system_template.empty()) tool_call_system_template_ = jinja::Parser::parse(tool_call_system_template);
        // if (!tool_call_grammar_template.empty()) tool_call_grammar_template_ = jinja::Parser::parse(tool_call_grammar_template);
    }
    ...
    void handle(jinja::Value & values) const {
        std::string grammar;
        ...
        // jinja::Value context = jinja::Value::context(json({
        //     // {"model", model_context},
        //     {"tools", tools},
        ...
        if (chat_epilogue_) {
            ...
            if (context.contains("grammar")) {
                grammar = context.get<std::string>("grammar", "");
            }
        }
    ...
    static std::unique_ptr<ChatHandler> find(const std::string & name, const json & model_context) {
        ...
        if (handler_idx < 0) {
            ...
            for (size_t i = 0; i < handlers.size(); ++i) {
                ...
                auto condition_eval = jinja::Parser::parse(handler["condition"]);
                jinja::Value context(json({{"model", model_context}}));

...
static void announce_test(const std::string & name, const jinja::Options & options) {
    auto len = name.size();
...
static void test_render(const std::string & template_str, const json & bindings, const jinja::Options & options, const std::string & expected, const json & expected_context = {}) {
...
static void test_error_contains(const std::string & template_str, const json & bindings, const jinja::Options & options, const std::string & expected) {
    announce_test(template_str, options);
    try {
        auto root = jinja::Parser::parse(template_str, options);
        auto context = jinja::Context::make(bindings);
